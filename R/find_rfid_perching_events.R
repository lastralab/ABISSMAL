...
#' 
#' @param df A data frame with the following columns: year, month, day, a column containing RFID timestamps, and a column containing the PIT tag identities. The function was written to accept data frames inside a list generated by splitting a larger data frame by PIT tag ID (e.g. one data frame per PIT tag ID is used as input)
#' @param rfid_col_nm A string with the column name for the RFID event timestamps. These timestamps must be in POSIXct or POSIXt format with millisecond resolution (e.g. format = "%Y-%m-%d %H:%M:%OS6")
#' @param tag_id_col_nm A string with the column name that contains PIT tag identifiers, so that perching events are identified for each unique PIT tag. This column name is used to group the data frame by unique PIT tags
