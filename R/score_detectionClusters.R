...
score_detectionClusters <- function(file_nm, rfid_label = NULL, camera_label = NULL, outer_irbb_label = NULL, inner_irbb_label = NULL, general_metadata_col_nms, video_metadata_col_nms, integrate_perching, sensor_id_col_nm = NULL, PIT_tag_col_nm = NULL, perching_dataset = NULL, perching_prefix = NULL, path, data_dir, out_dir, out_file_nm = "scored_detectionClusters.csv", tz, POSIXct_format = "%Y-%m-%d %H:%M:%OS"){
  ...
  expect_nonNulls <- f_args[grep(paste(paste("^", c("file_nm", "general_metadata_col_nms", "integrate_perching", "path", "data_dir", "out_dir", "out_file_nm", "tz", "POSIXct_format"), "$", sep = ""), collapse = "|"), names(f_args))]
  ...
  # Do not include the RFID related columns sensor_id_col_nm and PIT_tag_ID in the tests below of columns in the main input data. These are columns in the perching datasets that are generated by `find_perching_events`
  colnames_fArgs <- f_args[grep("col", names(f_args))][-grep("general_metadata_col_nms|sensor_id_col_nm|PIT_tag_col_nm", names(f_args[grep("col", names(f_args))]))]
