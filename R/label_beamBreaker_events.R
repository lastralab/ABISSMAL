...
#' 
#' @details Use pre-processed data from two pairs of beam breakers to label entrance and exit movements (in or out of a nest container or nesting area). This function uses temporal lags between the two beam breaker pairs and the given temporal thresholds to infer directionality of unique movement events by an animal entering or leaving a nest container or area. The outer pair of beam breakers is the first pair that should trigger when an animal enters a nest container with the 2 beam breakers mounted around the entrance. The inner pair of beam breakers should be the second pair that triggers when an animal enters, but the first pair to trigger when an animal leaves. This function must be executed across experimental setups if the tracking system was used to collect data for serial or parallel experimental replicates.
#'
