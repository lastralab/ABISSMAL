...
#' 
find_detectionClusters <- function(file_nms, threshold, run_length = 2, sensor_id_col, timestamps_col, PIT_tag_col, rfid_label, camera_label, drop_tag, preproc_metadata_cols, general_metadata_cols, video_metadata_cols, path, data_dir, out_dir, out_file_nm = "detection_clusters.csv", tz, POSIXct_format = "%Y-%m-%d %H:%M:%OS"){
  ...
  expect_strings <- f_args[-grep(paste(paste("^", c(expect_numeric), "$", sep = ""), collapse = "|"), names(f_args))]
  ...
  all_sensors <- data.table::rbindlist(lapply(1:length(file_nms), function(x){
    ...
    if(!any(grepl(paste(video_metadata_cols, collapse = "|"), names(tmp)))){
  ...
  if(any(grepl(rfid_label, unique(all_sensors$data_type)))){
