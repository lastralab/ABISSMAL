#' @title detect_rfid_activityBouts
#' @description Detect bouts of activities in integrated datasets with RFID timestamps
#'
...
#' @param data_dir A character string. This argument should be the name of directory where the integrated data is saved inside the path above. For instance, "integrated".
#' @param out_dir A character string. This argument should be the name of a directory specifying where the .csv file of activity bout data should be saved. For instance, "integrated". This folder will be appended to the data_path and created as a new directory if it doesn't already exist.
#' @param out_file_nm A character string. The name (plus extension) of the resulting file that will be written to out_dir. The default is "rfid_activity_bouts.csv".
...
detect_rfid_activityBouts <- function(integrated_file_nm, threshold, run_length = 2, timestamps_col, PIT_tag_col, preproc_metadata_cols, general_metadata_cols, path, data_dir, out_dir, out_file_nm = "rfid_activity_bouts.csv", tz, POSIXct_format = "%Y-%m-%d %H:%M:%OS"){
  ...
  colnames_fArgs <- f_args[grep("col", names(f_args))][-grep("preproc", names(f_args[grep("col", names(f_args))]))]
