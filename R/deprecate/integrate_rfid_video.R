...
#' @param preproc_metadata_cols A character vector. This argument should be a string of the metadata column names from pre-processing that should be dropped from either or both data frames. For instance, c("thin_threshold_s", "data_stage", "date_pre_processed", "pixel_threshold").
...
integrate_rfid_video <- function(rfid_file_nm, video_file_nm, l_th, u_th, video_rec_dur, sensor_id_col, timestamps_col, PIT_tag_col, preproc_metadata_cols, general_metadata_cols, video_metadata_cols, devices_integrated, extra_cols2drop, integrate_perching, path, data_dir, out_dir, out_file_nm = "integrated_rfid_video_data.csv", tz, POSIXct_format = "%Y-%m-%d %H:%M:%OS"){
  ...
  invisible(sapply(1:length(rfid_expected_cols), function(i){
    check_fArgs_data_cols(f_args[[grep(paste(paste("^", rfid_expected_cols[i], "$", sep = ""), collapse = "|"), names(f_args))]], preproc_rfid2)
  }))
  
  invisible(sapply(1:length(video_expected_cols), function(i){
    check_fArgs_data_cols(f_args[[grep(paste(paste("^", video_expected_cols[i], "$", sep = ""), collapse = "|"), names(f_args))]], preproc_video2)
  }))
  ...
  check_tstmps_cols(unlist(tstmps_cols), preproc_rfid2, "%Y-%m-%d %H:%M:%OS6")
  
  check_tstmps_cols(unlist(tstmps_cols), preproc_video2, "%Y-%m-%d %H:%M:%OS6")
