...
integrate_beamBreakers_video <- function(irbb_file_nm, video_file_nm, l_th = NULL, u_th = NULL, video_rec_dur = NULL, sensor_id_col, timestamps_col, PIT_tag_col, outer_irbb_col, inner_irbb_col, irbb_event_col, irbb_unique_col, general_metadata_cols, video_metadata_cols, path, irbb_dir, video_dir, out_dir, tz, POSIXct_format = "%Y-%m-%d %H:%M:%OS"){
  ...
  integr8d_df <- lags_df %>%
    ...
    dplyr::select(all_of(general_metadata_cols), all_of(outer_irbb_col), all_of(inner_irbb_col), all_of(video_col), all_of(irbb_event_col), all_of(irbb_unique_col), inner_video_diffs, assignmnt_type, movement_inference, all_of(video_metadata_cols), data_stage, irbb_video_lower_threshold_s, irbb_video_upper_threshold_s, video_recording_duration_s, date_integrated) %>% 
