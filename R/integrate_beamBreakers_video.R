...
#' @description Integrate pre-processed and labeled bream breaker detections with pre-processed video detections
#' 
...
#' @param preproc_metadata_cols A character vector. This argument should be a string of the metadata column names from pre-processing that should be dropped from either or both input data frames representing the pre-processed datasets prior to integration. For instance, c("pixel_threshold", "data_stage", "date_pre_processed", "lower_threshold_s", "upper_threshold_s", "date_labeled").
#' @param general_metadata_cols A character vector. This argument should be a string of the general metadata column names that will be included in the spreadsheet generated by this function. For instance: c("chamber_id", "year", "month", "day"). These columns will be added as the first columns in the integrated data frame, in the same order in which they are provided.
#' @param video_metadata_cols A character vector. This argument should be a string of the video metadata column names that will be included in the spreadsheet generated by this function. For instance: c("total_pixels_motionTrigger", "pixel_threshold", "video_file_name"). These columns will be added as later columns in the integrated data frame, in the same order in which they are provided.
...
#' 
#' @details This function uses lags between the pre-processed, labeled infrared beam breaker (IRBB) data and the pre-processed video data to integrate these two datasets. Each labeled beam breaker event (an entrance or an exit) in the resulting integrated dataset will be accompanied by a detection from the video dataset (e.g. the onset of a video recording event triggered by motion detection). This function was written to integrate data between the inner pair of beam breakers (from the pre-processed and labeled beam breaker dataset) and 1 camera mounted on a nest container, and will need to be updated/tested for datasets collected using other types of hardware setups. This function uses the temporal differences between the inner beam breaker timestamps and video timestamps that occurred within the lower and upper temporal thresholds to integrate these two datasets. Inner beam breaker timestamps that occurred after the onset of video recording are retained during this integration but are inferred to not be part of the original movement that triggered video recording (e.g. `irbb_video_movement_inference` = "post-motion_trigger"). During this integration, the same inner beam breaker timestamp will not be assigned to multiple video timestamps, regardless of whether this was motion or post-motion assignment. However, the same video timestamp can be assigned to different inner beam breaker timestamps, since some inner beam breaker detections may have happened after the original movement that triggered the video recording. This function integrates detections between datasets regardless of whether or not a given detection occurred during perching events captured by the RFID antenna (see `find_rfid_perching_events`). Some perching events may have started or ended as entrance or exit events, and it's important to retain those events at this stage. Perching event information can be returned by setting the argument `integrate_perching` to `TRUE`, which can subsequently be used to filter the integrated dataset as needed.
#' 
#' @return This function returns a spreadsheet in .csv format with the metadata columns from the original pre-processed data, as well as columns indicating each of the timestamps of the lead and rear beam breaker pairs, the timestamps of the video recording events, a unique label for the given event (e.g. entrance or exit), a unique numeric identifier for the given event, and information about the given data processing stage. Each row in the .csv file is a labeled event across the outer and inner beam breaker pairs that was integrated with video recording events (by matching each video timestamp to an inner beam breaker timestamp, as described above). Information about the temporal thresholds used for the integration and the date of integration is also contained in the resulting spreadsheet.
#' 

# TKTK here I need to change the inner pair to the outer for now for ABS talk graphics

integrate_beamBreakers_video <- function(irbb_file_nm, video_file_nm, l_th, u_th, video_rec_dur, sensor_id_col, timestamps_col, outer_irbb_col, inner_irbb_col, irbb_event_col, irbb_unique_col, preproc_metadata_cols, general_metadata_cols, video_metadata_cols, devices_integrated, extra_cols2drop, integrate_perching, path, data_dir, out_dir, out_file_nm = "integrated_beamBreaker_video_data.csv", tz, POSIXct_format = "%Y-%m-%d %H:%M:%OS"){
  ...
  if(devices_integrated == "two"){
    ...
    expect_strings <- f_args[-grep(paste(paste("^", c(expect_numeric, expect_bool, names(expect_na)), "$", sep = ""), collapse = "|"), names(f_args))]
  ...
  irbb_expected_cols <- colnames_fArgs[-grep(paste(names(video_expected_cols), collapse = "|"), names(colnames_fArgs))]
  ...
  invisible(sapply(1:length(md_cols), function(i){
    check_fArgs_data_cols(f_args[[grep(paste(paste("^", md_cols[i], "$", sep = ""), collapse = "|"), names(f_args))]], preproc_video)
  }))
  ...
  # TKTK changing to the outer pair for now: Here I'm interested in aligning to the inner pair of beam breakers only, since the camera sits last/in the sequence of sensors that should trigger for entrances/exits, and right after/before the inner pair of beam breakers
  lags_df <- video_df_tmp %>% 
    ...
    dplyr::mutate(
      leading_irbb = lead(!!sym(outer_irbb_col), default = first(!!sym(outer_irbb_col))),
      lagging_irbb = lag(!!sym(outer_irbb_col), default = first(!!sym(outer_irbb_col)))
    ) %>% 
    ...
    dplyr::mutate(
      ...
      inner_video_lead_diffs = round(as.numeric(leading_irbb - !!sym(video_col)), 2),
      inner_video_lag_diffs = round(as.numeric(lagging_irbb - !!sym(video_col)), 2),
  ...
  integr8d_df <- lags_df %>%
    ...
    dplyr::mutate(
      !!outer_irbb_col := leading_irbb,
      inner_video_diffs = inner_video_lead_diffs,
      irbb_video_assignmnt_type = "lead",
      irbb_video_movement_inference = "motion_trigger"
    ) %>% 
    # Add back metadata about the beam breaker event labels
    dplyr::inner_join(
      labeled_irbb2 %>%
        dplyr::select(all_of(outer_irbb_col), all_of(irbb_event_col), all_of(irbb_unique_col)),
    ...
    dplyr::select(all_of(outer_irbb_col), all_of(video_col), all_of(irbb_event_col), inner_video_diffs, irbb_video_assignmnt_type, irbb_video_movement_inference) %>%
    # Entrances, lag differences
    bind_rows(
      lags_df %>%
        dplyr::mutate(
          !!outer_irbb_col := lagging_irbb,
    ...
    bind_rows(
      lags_df %>%
        dplyr::mutate(
          ...
          inner_video_diffs = inner_video_lead_diffs,
          irbb_video_assignmnt_type = "lead",
          irbb_video_movement_inference = "post-motion_trigger"
        ) %>%
        # Add back metadata about the beam breaker event labels
        dplyr::inner_join(
          labeled_irbb2 %>%
  ...
  if(!all(is.na(extra_cols2drop))){
    ...
    integr8d_df <- integr8d_df %>% 
      ...
      dplyr::select(all_of(general_metadata_cols), all_of(outer_irbb_col), all_of(inner_irbb_col), all_of(video_col), all_of(irbb_event_col), all_of(irbb_unique_col), inner_video_diffs, irbb_video_assignmnt_type, irbb_video_movement_inference, all_of(video_metadata_cols), data_stage, irbb_video_lower_threshold_s, irbb_video_upper_threshold_s, all_of(extra_cols2drop), video_recording_duration_s, date_integrated) %>% 
      dplyr::arrange(!!sym(outer_irbb_col), desc = FALSE)
  ...
  dup_inds <- which(duplicated(integr8d_df[[outer_irbb_col]]))
  ...
  if(length(dup_inds) > 0){
    ...
    tmp_df <- data.table::rbindlist(lapply(1:length(dup_inds), function(i){
      ...
      tmp_dup <- integr8d_df %>% 
        slice(dup_inds[i]) %>% 
        pull(!!sym(outer_irbb_col))
      
      return(
        integr8d_df %>% 
          dplyr::filter(
            !!sym(outer_irbb_col) == tmp_dup
          ) %>% 
  ...
  if(integrate_perching){
    ...
    if(nrow(perch_df) > 0){
      ...
      integr8d_df_noDups <- integr8d_df_noDups %>% 
        dplyr::rename(
          `outer_irbb_col` = !!sym(outer_irbb_col)
        )
      
      tmp_df <- integr8d_df_noDups %>% 
        dplyr::select(outer_irbb_col) %>% 
        pmap_dfr(., function(outer_irbb_col){
          
          PIT_tag_col <- names(perch_df)[grep("PIT_tag", names(perch_df))]
          
          tmp_perching <- perch_df %>% 
            dplyr::filter(
              outer_irbb_col >= perching_start & outer_irbb_col <= perching_end 
            ) %>% 
            dplyr::mutate(
              outer_irbb_col = outer_irbb_col
            ) %>% 
            dplyr::select(outer_irbb_col, all_of(PIT_tag_col), perching_start, perching_end, perching_duration_s, unique_perching_event)
