...
In the code above, you created a vector called `x` that has 2 NA values. The function `is.na()` checks whether each element of `x` is equivalent to `NA`, and returns `TRUE` when that condition is met.
