...
Now you can update the plotting code to change the aesthetics of the x-axis using the function `scale_x_datetime()` to specify that you want x-axis labels every half hour. Also add an x-axis title:
```{r}
...
TKTK the color legend is added by default with the new geom_segment call, so make only minor changes from here on



Then end with adding the legend for color.


Finally, you need to add a legend to this plot so you can communicate what the two different colors mean. Adding this second legend to this plot is tricky because you manually coded the colors of the line segments in two separate uses in `geom_segment()`. `ggplot2` functions have specific requirements for making a legend, and they can require automated coding of aesthetic values like color.
