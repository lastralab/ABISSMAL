...
Next, you can check that `write.csv()` worked as expected by using `list.files()` to check the files in your working directory. In the second usage of `list.files()` below, you're using a new argument called `pattern` that makes it possible to customize the search. Using the argument `pattern` here is similar to searching for a specific word inside of a text document. Below, the dollar sign placed after ".csv" means that you're telling the function to search specifically for all files that end in the pattern ".csv".
```{r}
...
Now that you've practiced using `write.csv()`, you can delete the temporary file that you created by feeding the `rfid_file` object to the function `file.remove()`.
```{r eval = FALSE}
...
Check that the test file was created, and then delete it.
```{r eval = FALSE}
...
<h3>Write a loop to filter and save data frames</h3>

In order to write out a data frame for each day of data collection per sensor, you could repeat the code above 4 times (twice per sensor). But it's good coding habit to avoid repeating code over and over, since this can lead to messy scripts as well as greater risk for errors in data processing and analysis. Whenever you need to run the same code many times, you can write a loop. Loops are an important coding skill and it can take some time to wrap your head around them, so we'll work through building a loop step by step.

TKTK continue


you'll write out a spreadsheet per sensor for each day of data collection. You're writing out a spreadsheet per day in order to use the first ABISSMAL function, which combines data collected across days per sensor into a single spreadsheet and in the right format for downstream functions.

In order to write out a spreadsheet per sensor and day, you need to filter the rows of the full data frame for each sensor by date.
