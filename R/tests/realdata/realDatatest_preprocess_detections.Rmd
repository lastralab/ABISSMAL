...
# Error in preprocess_detections(df = .x, detection_col_nm, group_col_nm = "PIT_tag_ID", :
# The input object needs to be a data frame
pct_df3 %>% 
  dplyr::filter(data_type == "RFID") %>%
  map_dfr(
    ~ preprocess_detections(df = .x, detection_col_nm, group_col_nm = "PIT_tag_ID", threshold = 2)
  )
...
rfid_irbb_pre_proc <- pct_df3 %>% 
  ...
  bind_rows(
    pct_df3 %>% 
      ...
      # group_split(data_type) %>%
      map_dfr(
        ~ preprocess_detections(df = .x, detection_col_nm, group_col_nm = "data_type")
      )
  )
