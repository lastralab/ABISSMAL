...
test_that("The output is the same regardless of which sensor triggered first", {
  ...
  test_irbb <- data.frame(`Outer beam breaker` = outer_ts) %>% 
    dplyr::mutate(
      `Inner beam breaker` = outer_ts,
      chamber_id = "Box_01",
      year = year(outer_ts),
      month = month(outer_ts),
      day = day(outer_ts),
      sensor_id = c(rep("Outer beam breaker", length(outer_ts)),
                    rep("Inner beam breaker", length(outer_ts))),
      irbb_direction_inferred = rep(c("Entrance", "Exit"), 2),
      unique_entrance_exit = paste(rep(c("Entrance", "Exit"), 2), rep(seq(1, 2, 1), each = 2), sep = "-")
    )
  ...
  # Test the function
  integrate_rfid_beamBreakers(rfid_file_nm = "test_rfid.csv", irbb_file_nm = "test_irbb.csv", l_th = 0, u_th = 2, sensor_id_col = "sensor_id", timestamps_col = "timestamps_ms", outer_irbb_col = "Outer beam breaker", inner_irbb_col = "Inner beam breaker", irbb_event_col = "irbb_direction_inferred", irbb_unique_col = "unique_entranceExit", preproc_metadata_cols = NULL, general_metadata_cols = c("chamber_id", "year", "month", "day"), path = "/home/gsvidaurre/Desktop", integrate_perching = TRUE, data_dir = "tmp_tests", out_dir = "tmp_tests", out_file_nm = "integ_rfid_beamBreaker_testData.csv", tz = "America/New York", POSIXct_format = "%Y-%m-%d %H:%M:%OS")
  
  # Read in the output, check the output, then delete all files
  test_res <- read.csv(file.path(path, "integ_rfid_beamBreaker_testData.csv.csv"))
  # glimpse(test_res)
