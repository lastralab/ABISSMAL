...
# TKTK....how to handle perching events here?? Should these be entirely excluded since they are different behaviors? Or should the first timestamp per discrete perching event still count as a nest attendance event??? Right now I feel as though it should be the former, because this is how I started splitting behaviors out for the ms figures. And if they're separate, then these types of events can be combined later to count discrete activities per adult anyways

# TKTK
# This test will depend on the threshold used
test_that("The correct number of discrete events are retained after pre-processing", {
