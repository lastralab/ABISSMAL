...
# Purpose: A dummy master script to test the function in IRBB.py.
# This dummy script should pass parameters specific to the given recording chamber
# to the function in IRBB.py. Will use this dummy script to test the updated IRBB.py code on a Raspberry Pi
...
# Also, better to use add_event_detect() and callback(), since wait_for_edge apparently blocks the main thread
# and causes problems when trying to run other code at the same time

GPIO.setup(BEAM_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)

# From GPIO documentation: "An edge is the name of a transition from HIGH to LOW (falling edge)
# or LOW to HIGH (rising edge)"
